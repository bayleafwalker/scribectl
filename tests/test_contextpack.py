...
def test_pack_is_sha_stable(vault):
    a = build_pack(vault, "Scene 01-01")
    b = build_pack(vault, "Scene 01-01")
    assert a.markdown == b.markdown
    assert a.sha == b.sha


def test_sha_is_receipt_for_content(vault):
    """The embedded pack-sha must equal the hash of the pack minus the sha line."""
    pack = build_pack(vault, "Scene 01-01")
...
def test_pack_pulls_the_minimal_slice(vault):
    md = build_pack(vault, "Scene 01-01").markdown
...
def test_pack_flags_unresolved_scope(vault):
    md = build_pack(vault, "Scene 01-01").markdown
    assert "## ⚠ Unresolved scope links" in md
    assert "[[Lower Ashmarket]] — referenced but no note exists" in md


def test_pack_includes_prior_timeline_events(vault):
    md = build_pack(vault, "Scene 01-01").markdown
    assert "ration ledger" in md
    assert "officially recorded as contained" in md


def test_pack_rejects_unknown_scene(vault):
    with pytest.raises(ValueError):
        build_pack(vault, "Scene 99-99")
    with pytest.raises(ValueError):
        build_pack(vault, "The Mist")  # exists, but not a scene_card


def test_pack_matches_golden(vault, fixture_root):
    ...
    generated = build_pack(vault, "Scene 01-01").markdown
