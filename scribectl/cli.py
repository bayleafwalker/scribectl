...
def _digest_text(cfg: ProjectConfig, vault: Vault, ts: TemplateSet) -> str:
    ...
    return render_digest(d, cfg.name)


def _write_status(cfg: ProjectConfig, vault: Vault, ts: TemplateSet, rows) -> Path:
    """Status.md = the next-actions digest pinned atop the derived-state table.
    A cache regenerated whole every run and read back by nothing (both `status
    --write` and `next --write` produce it, so the two never diverge)."""
    dash = cfg.roots["control"] / "Status.md"
    body = [
        "---", "type: status_dashboard", "---", "",
        _digest_text(cfg, vault, ts), "",
        "---", "",
        f"# Status — {cfg.name}", "",
        f"_Generated on {date.today().isoformat()}. A cache: regenerated every "
        "run, read back by nothing. Do not edit — edit the vault._", "",
...
def cmd_status(args) -> int:
    ...
    if args.write:
        print(f"\nwrote {_write_status(cfg, vault, ts, rows)}")
    return 0
...
def cmd_next(args) -> int:
    ...
    vault = Vault.load(cfg.root)
    ts = _ts(cfg)
    print(_digest_text(cfg, vault, ts))
    if args.write:
        rows = project_rows(vault, ts)
        print(f"\nwrote {_write_status(cfg, vault, ts, rows)}")
    return 0
...
def _parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("next", help="next-actions digest: where the next ten minutes go")
    p.add_argument("-p", "--project")
    p.add_argument("--write", action="store_true",
                   help="also pin the digest atop control/Status.md")
